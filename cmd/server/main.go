...
import (
	...
	"github.com/infobloxopen/atlas-dapr-gateway/pkg/dapr"
	"github.com/infobloxopen/atlas-dapr-gateway/pkg/pb"
)
...
func main() {
	...
	s := strings.Split(viper.GetString("dapr.source"), ":")
	if len(s) != 2 {
		logger.Fatalf("dapr.source param should be the pair of semicolon-separated values, but defined as %q", viper.GetString("dapr.source"))
	}
	source := dapr.PubsubConfig{Name: s[0], Topic: s[1]}
	d := strings.Split(viper.GetString("dapr.destination.list"), ",")
	if len(d) < 1 {
		logger.Fatalf("dapr.destination.list param should be the list of comma-separated pairs, but it is empty")
	}

	destination := make([]dapr.PubsubConfig, 0)
	for i, v := range d {
		s := strings.Split(v, ":")
		if len(s) != 2 {
			logger.Fatalf("dapr.destination.list param should contain the pairs of semicolon-separated values, but pair %d defined as %q", i+1, v)
		}
...
func ServeExternal(logger *logrus.Logger) error {
	...
	s, err := server.NewServer(
		...
		server.WithGateway(
			...
			gateway.WithEndpointRegistration(viper.GetString("gateway.endpoint"), pb.RegisterAtlasDaprGatewayHandlerFromEndpoint),
		),
	...
	logger.Infof("serving gRPC at %s:%s", viper.GetString("server.address"), viper.GetString("server.port"))
	logger.Infof("serving http at %s:%s", viper.GetString("gateway.address"), viper.GetString("gateway.port"))
